...
[The recovery output implementation plan](recovery-output-implementation-plan.md) replaces the fixed 32 KiB ceiling with configurable byte and line ceilings while keeping 32 KiB as the byte default. Direct recovery commands bypass the ordinary Pi extension cap, so the query command itself remains the only owner of its output bound.
