...
A complete local validation across Pi, Codex, Claude Code, and Cursor imported 718,008 calls with no orphan records. The command-aware rules removed 68,232,604 characters from stored shell outputs. This is 24.7424% of eligible output and 3.51403% of all rendered output. The eligible set is broader than the earlier generic policy because more commands now have explicit rules. The generated database and transcripts are not included in the repository.
